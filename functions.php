<?php
  ...
  function my_admin_print_footer_scripts()
  {
      ...
      if (!in_array('my_pointer', $dismissed)):
          $pointer_content = '<h3>你好！验证 Gloom 主题成功</h3>';
  ...
  {
      if ($_GET[''.cs_get_option('i_login_prefix').''] != ''.cs_get_option('i_login_suffix').'') {
          header('Location: '.cs_get_option('i_login_link').'');
      }
  }
  ...
  /* 引入Sitemap.xml */
  if (cs_get_option('i_seo_sitemap')) {
      include('sitemap.php');
  }

  /* 引入百度主动推送 */
  if (cs_get_option('i_baidu_submit')) {
  ...
  //移除自动保存
  if (cs_get_option('i_post_autosave')) {
      wp_deregister_script('autosave');
  }

  //移除修订版本
  if (cs_get_option('i_post_revision')) {
