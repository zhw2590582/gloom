...
define('YOUR_LICENSE_SERVER_URL', 'http://wpGloom.duapp.com');
define('YOUR_ITEM_REFERENCE', 'island');
add_action('admin_menu', 'slm_Island_license_menu');
function slm_Island_license_menu() {
    add_options_page('Gloom密钥验证', 'Gloom密钥验证', 'manage_options', __FILE__, 'Island_license_management_page');
}
function Island_license_management_page() {
    echo '<div class="wrap">';
    echo '<h2>Gloom密钥验证</h2>';
    if (isset($_REQUEST['activate_license'])) {
        $license_key = $_REQUEST['Gloom_license_key'];
        $api_params = array(
        ...
        if($license_data->result == 'success'){//Success was returned for the license activation
            ...
            update_option('Gloom_license_key', $license_key);
    ...
    if (isset($_REQUEST['deactivate_license'])) {
        ...
        if($license_data->result == 'success'){
            echo '<div class="verify_serve">服务器返回信息: '.$license_data->message. '</div>';
            update_option('Gloom_license_key', '');
        }
        else{
            echo '<div class="verify_serve">服务器返回信息: '.$license_data->message. '</div>';
        }
	...
	<div class="verify_bg">
		...
		<form action="" method="post" class="verify_form">
			<input class="regular-text" autocomplete="off" type="password" id="license_key" name="Gloom_license_key"  placeholder="输入密钥信息" value="<?php echo get_option('Gloom_license_key'); ?>" >
